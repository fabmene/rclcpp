...
#include "rclcpp_components/msg/node_description.hpp"
...
void ComponentManager::OnLoadNode(const std::shared_ptr<rmw_request_id_t> request_header,
  ...
  try
  {
    ...
    for (const auto& resource : resources)
    {
      ...
      try
      {
        ...
        RCLCPP_INFO_STREAM(this->get_logger(), "Created node with id " << node_id << ", total: "
                                                                       << node_wrappers_.size());
      }
      catch (...)
      {
...
void ComponentManager::OnUnloadNode(const std::shared_ptr<rmw_request_id_t> request_header,
                                    ...
                                    std::shared_ptr<UnloadNode::Response> response)
{
  ...
  else
  {
    ...
    RCLCPP_INFO_STREAM(this->get_logger(), "" << request->unique_id << " deleted, "
                                              << node_wrappers_.size() << " nodes left");
    response->success = true;
  }
}

void ComponentManager::OnListNodes(const std::shared_ptr<rmw_request_id_t> request_header,
...
void ComponentManager::OnLoadNodeArray(
    ...
    std::shared_ptr<rclcpp_components::srv::LoadNodes::Response> response)
{
  ...
  if (auto exec = executor_.lock())
  {
    try
    {
      ...
      response->results.resize(request->node_descriptions.size());
      response->names.resize(request->node_descriptions.size());
      response->unique_ids.resize(request->node_descriptions.size());
      for (std::vector<rclcpp_components::msg::NodeDescription>::size_type i = 0;
           i < request->node_descriptions.size(); ++i)
      {
        const rclcpp_components::msg::NodeDescription& desc = request->node_descriptions[i];

        auto resources = get_component_resources(desc.package_name);

        bool lResourceFound = false;
        for (const auto& resource : resources)
        {
          if (resource.first != desc.plugin_name)
          {
            continue;
          }
          auto factory = create_component_factory(resource);
          ...
          std::vector<std::string> remap_rules;
          remap_rules.reserve(desc.remap_rules.size() * 2 + 1);
          remap_rules.push_back("--ros-args");
          for (const std::string& rule : desc.remap_rules)
          {
            remap_rules.push_back("-r");
            remap_rules.push_back(rule);
          }
          ...
          {
            remap_rules.push_back("-r");
            remap_rules.push_back("__node:=" + desc.node_name);
          }

          if (!desc.node_namespace.empty())
          {
            remap_rules.push_back("-r");
            remap_rules.push_back("__ns:=" + desc.node_namespace);
          }
          ...
          for (const auto& a : desc.extra_arguments)
          {
            const rclcpp::Parameter extra_argument = rclcpp::Parameter::from_parameter_msg(a);
            if (extra_argument.get_name() == "use_intra_process_comms")
            {
          ...
          if (0 == node_id)
          {
            ...
            // But even if you could add (and remove) them at 1 kHz (very optimistic rate)
            // it would still be a very long time before you could exhaust the pool of id's:
            //   2^64 / 1000 times per sec / 60 sec / 60 min / 24 hours / 365 days = 584,942,417
            //   years
            // So around 585 million years. Even at 1 GHz, it would take 585 years.
            // I think it's safe to avoid trying to handle overflow.
            // If we roll over then it's most likely a bug.
          ...
          {
            node_wrappers_[node_id] = factory->create_node_instance(options);
            RCLCPP_INFO_STREAM(this->get_logger(), "Created node with id "
                                                       << node_id
                                                       << ", total: " << node_wrappers_.size());
          }
          catch (...)
          {
            // In the case that the component constructor throws an exception,
          ...
          exec->add_node(node, true);
          response->names[i] = node->get_fully_qualified_name();
          response->unique_ids[i] = node_id;
          response->results[i] = true;
          break;
        }
        ...
        if (!lResourceFound)
        {
          RCLCPP_ERROR(get_logger(),
                       "Failed to find class with the requested plugin name '%s' in "
                       "the loaded library",
                       desc.plugin_name.c_str());
          response->results[i] = false;
        }
      }
    }
    catch (const ComponentManagerException& ex)
...
void ComponentManager::OnUnloadNodeArray(
    const std::shared_ptr<rmw_request_id_t> request_header,
    ...
    std::shared_ptr<rclcpp_components::srv::UnloadNodes::Response> response)
{
  ...
  if (auto exec = executor_.lock())
  {
    response->results.resize(request->unique_ids.size());
    for (std::vector<uint64_t>::size_type i = 0; i < request->unique_ids.size(); ++i)
    {
      uint64_t unique_id = request->unique_ids[i];
      auto wrapper = node_wrappers_.find(unique_id);

      if (wrapper == node_wrappers_.end())
      {
        response->results[i] = false;
        RCLCPP_WARN_STREAM(get_logger(), "No node found with unique_id: " << unique_id);
      }
      else
      {
        exec->remove_node(wrapper->second.get_node_base_interface());
        node_wrappers_.erase(wrapper);
        response->results[i] = true;
        RCLCPP_INFO_STREAM(this->get_logger(), "" << unique_id << " deleted, "
                                                  << node_wrappers_.size() << " nodes left");
      }
    }
  }
